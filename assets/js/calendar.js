let date=new Date(); // creates a new date object with the current date and time
    ...
    // Check if there are only 5 weeks displayed in the calendar
    const weeks = Math.ceil((dayone + lastdate) / 7);
    if (weeks === 5) {
        // Calculate the number of days needed to complete the additional week
        const daysToAdd = 7 - dayend;
        // Add additional days from the next month to complete the additional week
        for (let i = 1; i <= daysToAdd+6; i++) {
